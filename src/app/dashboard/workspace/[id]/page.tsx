...
export default async function WorkspacePage({ params}: WorkspacePageProps) {
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      >
        <CreateTaskDialog workspaceId={workspaceId} members={members} userId={Number.parseInt(session.user.id)} />
      </DashboardHeader>
        ...
        <TabsContent value="active" className="mt-4">
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            {activeTasks.length > 0 ? (
              activeTasks.map((task) => <TaskCard key={task.id} task={task} workspaceId={workspaceId} />)
            ) : (
        ...
        <TabsContent value="completed" className="mt-4">
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            {completedTasks.length > 0 ? (
              completedTasks.map((task) => <TaskCard key={task.id} task={task} workspaceId={workspaceId} />)
            ) : (
              <div className="col-span-full text-center py-8">
