...
export function TaskCard({ task, workspaceId, onDelete }: TaskCardProps) {
    ...
    <Card className={cn("transition-all", isCompleted ? "opacity-75" : "")}>
      <CardContent className="p-4">
        ...
        <div className="mt-4 flex flex-wrap items-center gap-2">
          {task.priority && (
            <span className={cn("rounded-full px-2 py-1 text-xs font-medium", getPriorityClass(task.priority))}>
              {task.priority}
            </span>
          )}
        ...
        <CardFooter className="border-t p-4 flex justify-between items-center">
          <div className="flex items-center gap-2">
            <Avatar className="h-6 w-6">
              <AvatarImage src={task.assigned_to_image || ""} alt={task.assigned_to_name || ""} />
              <AvatarFallback>{task.assigned_to_name?.charAt(0) || "U"}</AvatarFallback>
            </Avatar>
